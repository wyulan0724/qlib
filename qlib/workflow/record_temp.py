...
class RecordTemp:
    ...
    artifact_path = None
    # the dependant class of the record; the record will depend on the results generated by
    depend_cls = None
    # `depend_cls`
...
class SignalRecord(RecordTemp):
    ...
    def generate_label(dataset):
        with class_casting(dataset, DatasetH):
            params = dict(segments="test", col_set="label",
    ...
    def generate(self, *args, **kwargs):
        ...
        if self.skip_existing:
            try:
                ...
                logger.info(
                    "The results has previously generated, Generation skipped.")
                return
        ...
        try:
            ...
            logger.warning(
                "The dependent data does not exists. Generation skipped.")
            return
...
class PortAnaRecord(ACRecordTemp):
    ...
    def _get_report_freq(self, executor_config):
        ...
        if executor_config["kwargs"].get("generate_portfolio_metrics", False):
            _count, _freq = Freq.parse(
                executor_config["kwargs"]["time_per_step"])
            ret_freq.append(f"{_count}{_freq}")
        if "inner_executor" in executor_config["kwargs"]:
            ret_freq.extend(self._get_report_freq(
    ...
    def _generate(self, **kwargs):
        ...
        for k in "executor_config", "strategy_config":
            setattr(self, k, fill_placeholder(
                getattr(self, k), placeholder_value))
        ...
        if self.backtest_config["end_time"] is None:
            self.backtest_config["end_time"] = get_date_by_shift(
                dt_values.max(), 1)
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq not in portfolio_metric_dict:
                ...
                pprint(risk_analysis(
                    report_normal["bench"], freq=_analysis_freq))
                pprint(
        ...
        for _analysis_freq in self.indicator_analysis_freq:
            if _analysis_freq not in indicator_dict:
                warnings.warn(
                    f"the freq {_analysis_freq} indicator is not found")
            else:
    ...
    def list(self):
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq in self.all_freq:
                ...
                warnings.warn(
                    f"risk_analysis freq {_analysis_freq} is not found")
        ...
        for _analysis_freq in self.indicator_analysis_freq:
            if _analysis_freq in self.all_freq:
                ...
                warnings.warn(
                    f"indicator_analysis freq {_analysis_freq} is not found")
        return list_path
...
class MultiPassPortAnaRecord(PortAnaRecord):
    ...
    def _generate(self, **kwargs):
        ...
        for i in trange(self.pass_num):
            ...
            for _analysis_freq in self.risk_analysis_freq:
                risk_analysis_df_list = risk_analysis_df_map.get(
                    _analysis_freq, [])
        ...
        for _analysis_freq in self.risk_analysis_freq:
            ...
            multi_pass_port_analysis_df = combined_df.groupby(level=[0, 1]).apply(
                lambda x: pd.Series(
                    {"mean": x["risk"].mean(), "std": x["risk"].std(
    ...
    def list(self):
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq in self.all_freq:
                list_path.append(
                    f"multi_pass_port_analysis_{_analysis_freq}.pkl")
            else:
                warnings.warn(
                    f"risk_analysis freq {_analysis_freq} is not found")
        return list_path
